define(['template', 'plugins/seq/assets/seq'], function () {
  ...
  self.container = $('.product-skus');
  self.skuControl = $('.sku-control-form-group');
  self.skuTable = $('.sku-table');
  self.data = {};
  self.skus = [];
    ...
    self.form
      ...
      .ajaxForm({
        ...
        beforeSend: function (jqXHR, settings) {
          if (!self.isSinglePrice()) {
            settings.data += '&' + $.param(self.getSkuData());
          }
        },
   ...
   * 获取规格配置和规格详细信息
   */
  self.getSkuData = function () {
    return {
      skuConfigs: self.getSkuConfigs()
    };
  };
   ...
   * 生成笛卡尔积
   */
  self.cartesianProductOf = function () {
