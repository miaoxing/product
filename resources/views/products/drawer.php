<div class="js-product-drawers snap-drawers product-drawers bg-light display-none">
  <div class="snap-drawer snap-drawer-left category-drawer border-right">
    ...
    <?php if ($setting('products.categoryLevel') == '1') : ?>
      <ul class="category-list-sm">
        <li class="border-bottom">
          <a href="<?= $url('products') ?>">
            <h4 class="text-active-primary border-active-primary category-title <?= !$req['categoryId'] ? 'active' : '' ?>">全部</h4>
          </a>
        </li>
        <?php foreach ($categories as $category) : ?>
          <li class="border-bottom">
            <a href="<?= $url->query('products', ['categoryId' => $category['id']]) ?>">
              <h4
                class="text-active-primary border-active-primary category-title <?= $req['categoryId'] == $category['id'] ? 'active' : '' ?>"><?= $category['name'] ?></h4>
            </a>
          </li>
        <?php endforeach ?>
      </ul>
    <?php else : ?>
      ...
      <?php foreach ($categories as $category) : ?>
        <h4 class="js-category-title category-title border-active-primary text-active-primary border-bottom"><?= $category['name'] ?></h4>
          ...
          <?php foreach ($category->getChildren()->enabled()->notDeleted() as $childCategory) : ?>
            <li>
              <a class="border-bottom"
                 href="<?= $url->query('products', ['categoryId' => $childCategory['id']]) ?>"><?= $childCategory['name'] ?></a>
            </li>
          <?php endforeach ?>
        </ul>
      <?php endforeach ?>
